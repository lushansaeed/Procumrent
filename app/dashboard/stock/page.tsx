    ...
    <div className="space-y-6">
      ...
      {lowCount > 0 && (
        <Card className="border-yellow-200 bg-yellow-50">
          <CardContent className="p-4 text-sm text-yellow-800 font-medium">⚠ {lowCount} item{lowCount > 1 ? "s are" : " is"} at or below reorder level.</CardContent>
        </Card>
      )}
      <Card>
        ...
        <CardContent className="p-0">
          {stocks.length === 0 ? (
            <div className="flex flex-col items-center py-12 text-gray-400"><Package className="w-10 h-10 mb-2 opacity-30" /><p className="text-sm">No stock records yet</p></div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="border-b bg-gray-50 text-gray-500 text-xs uppercase tracking-wide"><th className="px-6 py-3 text-left">Item</th><th className="px-6 py-3 text-left">Category</th><th className="px-6 py-3 text-left">Location</th><th className="px-6 py-3 text-right">Qty</th><th className="px-6 py-3 text-right">Reserved</th><th className="px-6 py-3 text-right">Available</th><th className="px-6 py-3 text-right">Reorder At</th><th className="px-6 py-3 text-left">Status</th></tr></thead>
                <tbody className="divide-y">
                  {stocks.map(s => {
                    const st = stockStatus(s.quantity, s.item.reorderLevel, s.item.minStockLevel);
                    return (
                      <tr key={s.id} className="hover:bg-gray-50">
                        <td className="px-6 py-3"><p className="font-medium text-gray-900">{s.item.name}</p><p className="text-xs text-gray-400">{s.item.code}</p></td>
                        ...
                        <td className="px-6 py-3 text-right font-semibold">{s.quantity - s.reserved}</td>
