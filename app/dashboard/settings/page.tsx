...
import { Settings, Tag, MapPin, Users, Sliders } from "lucide-react";
...
export default async function SettingsPage() {
  ...
  const [categories, locations, employees] = await Promise.all([
    ...
    db.employee.findMany({
      where: { procurementRole: { not: null } },
      orderBy: { name: "asc" },
    }),
  ]);
              ...
              {categories.length === 0 ? (
                ...
                categories.map((cat) => (
                  ...
                  >
                    ...
                    <Badge
                      className={cat.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}
                    >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            ...
            <Button variant="outline" size="sm" asChild>
              <Link href="/dashboard/locations">Manage</Link>
            </Button>
          </CardHeader>
          <CardContent>
            <div className="space-y-1 max-h-60 overflow-y-auto">
              {locations.length === 0 ? (
                ...
                locations.map((loc) => (
                  ...
                  >
                    ...
                    <Badge
                      className={loc.isActive ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}
                    >
                      {loc.type}
                    </Badge>
                  </div>
          ...
          <CardContent>
            {employees.length === 0 ? (
              <p className="text-sm text-gray-400 py-4 text-center">
                No employees have been assigned procurement roles yet.
              </p>
            ) : (
                  ...
                  <tbody>
                    {employees.map((emp) => (
                      <tr key={emp.id} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-gray-500">{emp.department ?? "—"}</td>
                        <td className="px-4 py-2">
                          <Badge className="bg-purple-50 text-purple-700">{emp.procurementRole}</Badge>
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-4 rounded-lg border bg-gray-50">
                ...
                <p className="mt-1 text-sm font-medium">MVR (Maldivian Rufiyaa)</p>
              </div>
              <div className="p-4 rounded-lg border bg-gray-50">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Request Numbering</p>
                <p className="mt-1 text-sm font-medium">PR-YYYY-NNNN</p>
              </div>
              <div className="p-4 rounded-lg border bg-gray-50">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">PO Numbering</p>
                <p className="mt-1 text-sm font-medium">PO-YYYY-NNNN</p>
              </div>
              <div className="p-4 rounded-lg border bg-gray-50">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">GRN Numbering</p>
                <p className="mt-1 text-sm font-medium">GRN-YYYY-NNNN</p>
              </div>
            </div>
          </CardContent>
        </Card>
